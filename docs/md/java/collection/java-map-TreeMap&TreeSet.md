...
::: tip 提示
本文主要对Map - TreeSet & TreeMap 源码解析。 @123
...
> ![TreeMap_base.png](images/TreeMap_base.png)
...
> ![TreeMap_rotateLeft.png](images/TreeMap_rotateLeft.png)
...
> ![TreeMap_rotateRight.png](images/TreeMap_rotateRight.png)
...
> ![TreeMap_successor.png](images/TreeMap_successor.png)
...
> `get(Object key)`方法根据指定的`key`值返回对应的`value`，该方法调用了`getEntry(Object key)`得到相应的`entry`，然后返回`entry.value`。因此`getEntry()`是算法的核心。算法思想是根据`key`的自然顺序(或者比较器顺序)对二叉查找树进行查找，直到找到满足
...
> ![TreeMap_getEntry.png](images/TreeMap_getEntry.png)
...
> ![TreeMap_put.png](images/TreeMap_put.png)
...
> `remove(Object key)`的作用是删除`key`值对应的`entry`，该方法首先通过上文中提到的`getEntry(Object key)`方法找到`key`值对应的`entry`，然后调用

```java
deleteEntry(Entry<K,V> entry)
```

删除对应的`entry`。由于删除操作会改变红黑树的结构，有可能破坏红黑树的约束条件，因此有可能要进行调整。

> `getEntry()`函数前面已经讲解过，这里重点放`deleteEntry()`上，该函数删除指定的`entry`并在红黑树的约束被破坏时进行调用
...
> ![TreeMap_fixAfterDeletion.png](images/TreeMap_fixAfterDeletion.png)
